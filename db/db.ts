...
export async function getBestBreadcrumbs(
    tags: MinimalTag[],
    allPathsByName: Record<string, DbPlainTag[][]>
): Promise<BreadcrumbItem[]> {
    console.log("tags", tags)
    // don't consider tags that are parents of other tags
    // e.g. if we have "Energy" and "Nuclear Energy", we only want to evalulate "Nuclear Energy"
    // const filtered = tags.filter((tag) => {
    //     const isParentOfOtherTag = tags.some((otherTag) => {
    //         const paths = allPathsByName[otherTag.name]
    //         return (
    //             paths &&
    //             paths.some((path) =>
    //                 path.some((parent) => parent.id === tag.id)
    //             )
    //         )
    //     })
    //     return !isParentOfOtherTag
    // })
    ...
    for (const tag of tags) {
        const paths = allPathsByName[tag.name]
    ...
    const longestPath = Array.from(result.values()).reduce((best, path) => {
        return path.length > best.length ? path : best
    }, [])

    console.log("longestPath", longestPath)

    const topicsOnly = longestPath.filter((tag) => tag.slug)

    const breadcrumbs = topicsOnly.map((tag) => ({
        label: tag.name,
